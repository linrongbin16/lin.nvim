...
- `init`: Wrapped with `lua_init` method (for lua files) or `vim_init` method (for vim scripts).
- `config`: Wrapped with `lua_config` method (for lua files) or `vim_config` method (for vim scripts).
- `keys`: Wrapped with `lua_keys` method. Note: `keys` option doesn't support vim scripts.
...
To add a new plugin, please add a `lua/plugins/users.lua` file that follows the same structure of the `lua/plugins/init.lua`. You can simply copy and rename the sample file `lua/plugins/users_sample.lua` to enable it, it already has a lot of other recommended plugins, which may suit your needs.

To disable an embedded plugin, please add a `lua/disabled_plugins.lua` file that contains a set of plugin names formatted in `organization/repository`. You can simply copy and rename the sample file `lua/disabled_plugins_sample.lua` to enable it.

?> Check out `lua/configs/folke/lazy-nvim/config.lua` for how lazy.nvim loading the plugins list.

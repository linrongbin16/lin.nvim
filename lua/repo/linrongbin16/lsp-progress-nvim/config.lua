...
local function progress_handler(err, msg, ctx)
	...
	if value.kind then
		...
		local pm = STATE.data[client_id].progress_messages
		...
		elseif value.kind == "report" then
			...
			pm[token].spinner_index = (pm[token].spinner_index + 1) % SPINNER_LENGTH + 1
		...
		STATE.redrawed = false
		table.insert(STATE.data.once_messages, { client_id = client_id, content = value, shown = 0 })
	end

	-- if last redraw in update time threshold, skip this redraw
	if STATE.redrawed then
		return
	end
	...
	STATE.redrawed = true
	vim.defer_fn(reset_last_redraw, UPDATE_TIME)
end
...
local function progress()
	...
	local remove_once = {}
	for client_id, data in pairs(STATE.data) do
	...
	for _, item in ipairs(remove_once) do
		table.remove(STATE.data[item.client_id].once_messages, item.index)
	end
	for _, item in ipairs(remove_progress) do
		STATE.data[item.client_id].progress_messages[item.token] = nil
	end
	...
	for i, msg in ipairs(new_messages) do
		...
		if msg.progress_message then
			if msg.spinner_index then
				table.insert(builder, SPINNER[msg.spinner_index])
			end
	...
	return SIGN .. " " .. table.concat(buffer, " ┆ ")
end

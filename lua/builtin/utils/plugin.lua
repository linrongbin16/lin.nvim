local function lua_keys(keys)
    ...
    if not keys_list_ok then
        error("Error! Failed to load lua module '" .. keys_path .. "'!")
    end
    return keys_list
end
...
local function lua_init(init)
    return function()
        ...
        if not ok then
            error("Error! Failed to load lua module '" .. init_path .. "'!")
        end
        return init_module
    end
end
...
local function vim_init(init)
    return function()
        ...
        if vim.fn.filereadable(init_path) <= 0 then
            error("Error! Failed to load vimscript '" .. init_path .. "'!")
        end
...
local function lua_config(config)
    return function()
        ...
        if not ok then
            error("Error! Failed to load lua module '" .. config_path .. "'!")
        end
...
local function vim_config(config)
    return function()
        ...
        if vim.fn.filereadable(config_path) <= 0 then
            error("Error! Failed to load vimscript '" .. config_path .. "'!")
        end
